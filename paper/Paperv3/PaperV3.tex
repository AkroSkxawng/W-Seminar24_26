...
Für eine spätere praktische Implementierung des Sensors wie in Kapitel 4.2 angedeutet, wird für einen ersten Versuch die 2. Generation des Sensorlayouts verwendet werden.\newline
...
Aufgrund der Auflösung des für die Maske verwendeten 3D-Druckers und der Pinseldicke sowie durch den händischen Prozess zu erwartende Toleranzen wird die minimal erreichbare Größe von aufgebrachten Merkmalen als $0.4\,mm$ bis $0.5\,$ angenommen.\newline
...
Das Sensorpaket wird im Anschluss zum Beispiel durch ein vorbereitetes Case transportfähig gemacht und anschließend mittig zwischen den Helmholtz-Spulen positioniert, wo anschließend die Hall-Spannungen für die in Kapitel 4.2 angegebenen Magnetfelder gemessen werden.
...
Aufgrund der statischen Natur der Simulationen können primär die Sensitivität und Sensoroffset verglichen werden. Die Sensitivität eines Hall-Sensors kann nach \cite{ceidari15} mit 
\begin{equation}
	{S_{A_V}}=\frac{U_{Hall}}{B}
	\tag{5.1}
	\label{5.1}
\end{equation}
...
\begin{figure}
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			clip=false,
			xlabel={Zeit in $s$},
			ylabel={Magnetfeldstärke in $\mu{T}$},
			xmin=0, 
			ymin=-100, 
...
\begin{figure}
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			clip=false,
			xlabel={Zeit in $s$},
			ylabel={Magnetfeldstärke in $G\triangleq100\mu{T}$},
			xmin=0, 
			ymin=-1, 
			...
			table[x= Time (s) Run 1,y= |Resultant| Magnetic Field Strength (G) Run 1,col sep=comma, each nth point=15]{Measurements 12.09.2025 Pasco sensor.csv};
			\addplot [
			color=red
			] 
			table[y= X Magnetic Field Strength (G) Run 1,col sep=comma, each nth point=15]{Measurements 12.09.2025 Pasco sensor.csv};
			\addplot [
			color=green
			] 
			table[y= Y Magnetic Field Strength (G) Run 1,col sep=comma, each nth point=15]{Measurements 12.09.2025 Pasco sensor.csv};
			\addplot [
			color=orange
			] 
			table[y= Z Magnetic Field Strength (G) Run 1,col sep=comma, each nth point=15]{Measurements 12.09.2025 Pasco sensor.csv};			
			\legend{$B_{Ges}$, $B_X$, $B_Y$, $B_Z$}	
		\end{axis}
	\end{tikzpicture}
	\captionsetup{type=graph,name=Graph}
...
\begin{center}
	\footnotesize
	$B_{noise}$: Feld-Rauschgrenze $V_{n,out}$: Output-Spannungs-Rauschen, $S_B$: Sensorempfindlichkeit, \newline
	$G$ = Verstärker-Gain
\end{center}
